...
namespace Lab3
{
    public class QueryBuilder : IDisposable
    {
        ...
        public void Create<T>(T obj) where T : IClassModel
        {
            ...
            var command = connection.CreateCommand();
            //because of formatting, everything in the parenthesis needs to be changed manually to fit the parameters of the database you're using
            command.CommandText = $"INSERT INTO {typeof(T).Name} (DexNumber, Name, Form, Type1, Type2, Total, HP, Attack, Defense, SpecialAttack, SpecialDefense, Speed, Generation)  VALUES ({sb})";
